#pragma once

namespace my_model {

    struct cube {

        float coords[6 * 36] = {
                -1.0f,  1.0f, -1.0f,  0, 0, -1, // 1
                -1.0f, -1.0f, -1.0f,  0, 0, -1, // 4
                1.0f, -1.0f, -1.0f,  0, 0, -1, // 8
                1.0f, -1.0f, -1.0f,  0, 0, -1, // 8
                1.0f,  1.0f, -1.0f,  0, 0, -1,  // 5
                -1.0f,  1.0f, -1.0f,  0, 0, -1, // 1

                -1.0f, -1.0f,  1.0f, -1, 0, 0, // 3
                -1.0f, -1.0f, -1.0f, -1, 0, 0, // 4
                -1.0f,  1.0f, -1.0f, -1, 0, 0, // 1
                -1.0f,  1.0f, -1.0f, -1, 0, 0, //
                -1.0f,  1.0f,  1.0f, -1, 0, 0, //
                -1.0f, -1.0f,  1.0f, -1, 0, 0, //

                1.0f, -1.0f, -1.0f, 1, 0, 0, // 8
                1.0f, -1.0f,  1.0f, 1, 0, 0, // 7
                1.0f,  1.0f,  1.0f, 1, 0, 0, // 5
                1.0f,  1.0f,  1.0f, 1, 0, 0, //
                1.0f,  1.0f, -1.0f, 1, 0, 0, //
                1.0f, -1.0f, -1.0f, 1, 0, 0, //

                -1.0f, -1.0f,  1.0f, 0, 0, 1,// 3
                -1.0f,  1.0f,  1.0f, 0, 0, 1,// 2
                1.0f,  1.0f,  1.0f, 0, 0, 1,// 6
                1.0f,  1.0f,  1.0f, 0, 0, 1,
                1.0f, -1.0f,  1.0f, 0, 0, 1,
                -1.0f, -1.0f,  1.0f, 0, 0, 1,

                -1.0f,  1.0f, -1.0f, 0, 1, 0, // 1
                1.0f,  1.0f, -1.0f, 0, 1, 0, // 5
                1.0f,  1.0f,  1.0f, 0, 1, 0, // 6
                1.0f,  1.0f,  1.0f, 0, 1, 0,
                -1.0f,  1.0f,  1.0f, 0, 1, 0,
                -1.0f,  1.0f, -1.0f, 0, 1, 0,

                -1.0f, -1.0f, -1.0f, 0, -1, 0,
                -1.0f, -1.0f,  1.0f, 0, -1, 0,
                1.0f, -1.0f, -1.0f, 0, -1, 0,
                1.0f, -1.0f, -1.0f, 0, -1, 0,
                -1.0f, -1.0f,  1.0f, 0, -1, 0,
                1.0f, -1.0f,  1.0f, 0, -1, 0,
        };
    };

}
